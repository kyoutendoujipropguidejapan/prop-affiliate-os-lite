# PUBLIC LP TRIPLE FACT-CHECK COVERAGE MATRIX

更新日：2026-08-26 JST
Status：FIRST-PASS HIGH-RISK COVERAGE COMPLETE / FULL ROUTE AUDIT CONTINUES
Standard：`FACT_CHECK_STANDARD_V1_2026-08-26.md`

## Purpose

公開中LPに含まれる14社の高リスクFactについて、最低3回ファクトチェックの遡及監査がどこまで到達したかを一覧化する。

`covered` は「全Factが正しい」という意味ではない。高リスク項目の一次監査が実施され、`TRIPLE_VERIFIED / CORRECTION_REQUIRED / HOLD / CONFLICT / UPDATE_CANDIDATE` に分類されたことを意味する。

---

| Firm | High-risk audit | Main current status |
|---|---|---|
| Fintokei | Wave 1 / 2 / 8 | SwiftTrader cohort correction required; NEW20 Japan scope pending |
| Fundora | Wave 8 + local campaign review | Core rules verified; Professional/Master official price surfaces conflict; FND25 local accepted not Production-confirmed |
| SuperFunded | Wave 1 / 3 | Audited core values supported; selected payout fields remain implementation-time recheck items |
| Trading Cult Pro | Wave 6 | Current family/MT5 verified; Instant/Pay After Pass materially aligned; some third-independent-source gaps remain |
| Blueberry Funded | Wave 3 | Instant Lite current cohort correction required; Prime minimum-day wording update candidate |
| Blueberry Futures | Wave 2 / 7 | Ascent core verified; Accelerated consistency correction; Accelerated price conflict/HOLD |
| FundedElite | Wave 4 + Flash HOLD docs | Pay-after-pass structure verified; Flash default-vs-custom option matrix remains HOLD |
| Funded7 | Wave 2 / 4 + HOLD docs | WELCOME scoped discounts verified; One Phase/Instant HOLD; PAYG 4/8 vs 5/10 official conflict |
| Funded Trader Markets | Wave 1 / 4 + Instant Pro HOLD docs | Japanese support correction; Nitro X reward correction; Instant Pro official conflict remains HOLD |
| Blue Guardian | Wave 3 | 1 Step target cohort correction; 2 Step min-day cohort correction; Nano content gap |
| Hantec Trader | Wave 3 + HOLD recheck | Instant Lite HOLD resolved to VERIFIED_WITH_CAUTION; Production wording update pending |
| The5ers | Wave 1 / 4 | Home Summer 100K scoped card verified; Summer 200K preserved, dynamic official recheck pending |
| The5ers Futures | Wave 7 | Current EN 4%/40%/$59 strongly supported; locale conflict remains on Max Loss; price update not automatic |
| FundingPips | Wave 5 | Core five models verified; Standard reward-cycle wording and Zero profitable-day threshold update candidates |

---

## Cross-site/high-risk route coverage already included

- Home firm cards / purpose routing
- Current campaign/coupon block — partial, with exclusive-code evidence gaps preserved
- `/prop-firm-payout-comparison`
- `/pay-after-pass-payg`
- `/one-step-two-step-instant` scoped structure checks
- current Firm rule cards
- selected Futures surfaces
- public freshness metadata

---

## Not yet equivalent to full-site completion

The following still require route-by-route audit before the entire public estate can be marked retrospectively triple-verified:

1. every beginner/article route containing named Firm numeric facts
2. every coupon/history/current-campaign claim
3. every FAQ answer with Firm-specific numbers
4. every price display / price history value
5. every affiliate-exclusive code effect and scope where direct operational evidence is not yet captured
6. every source link still pointing to superseded official pages
7. page-level verification/freshness labels
8. actual iPhone production rendering and CTA-level PR disclosure

---

## Affiliate-exclusive code boundary

Current public site states a number of operator-confirmed exclusive codes.

Public web search not finding an exclusive code is **not** evidence that the code is invalid. Such codes may only exist in partner systems, checkout, or direct partner communication.

They must therefore be verified by one or more of:
- live purchase/checkout acceptance,
- partner portal/current campaign record,
- direct official partner confirmation,
- retained immutable evidence packet.

Until sufficient evidence is captured, do not strengthen effect/scope beyond the existing operator-confirmed wording.

---

## Current first-pass conclusion

All 14 currently represented Firms have now received at least one high-risk retrospective audit pass under the three-check standard.

This is sufficient to build a controlled correction queue, but **not** sufficient to declare the full public site `TRIPLE_VERIFIED`.

Final Status：
`ALL_14_FIRMS_HIGH_RISK_FIRST_PASS_COVERED / ROUTE_LEVEL_RETROSPECTIVE_AUDIT_CONTINUES`
